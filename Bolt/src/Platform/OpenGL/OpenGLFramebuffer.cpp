...
namespace Bolt {
	static const uint32_t s_MaxFrameBufferSize = 8192; // TODO: Get this from GPU look up
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height) {
		if (width <= 0 || height <= 0 || height > s_MaxFrameBufferSize || width > s_MaxFrameBufferSize) {
			BL_CORE_WARN("Attempted to resize framebuffer to {0} x {1}", width, height);
			return;
		}
